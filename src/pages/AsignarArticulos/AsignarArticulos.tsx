...
export const AsignarArticulos: React.FC = () => {
  ...
  useEffect(() => {
    const fetchResumen = async () => {
      ...
      try {
        setLoading(true);
        setError(null);
        const resumenData = await getResumenAsignados(proyectoId);
        setResumen(resumenData);
      } catch (err: any) {
        console.error('Error fetching resumen asignados:', err);
        setError('Error al cargar el resumen de asignaciones');
        toast.error('Error al cargar el resumen de asignaciones');
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleConfirmarUsuario = async () => {
    ...
    if (
      ...
      toast.error('Debe especificar un número válido de artículos');
      return;
    }
    ...
    try {
      ...
      toast.success('Asignación por usuario confirmada');
  ...
  const handleConfirmarFecha = async () => {
    ...
    try {
      ...
      toast.success('Asignación por fecha confirmada');
    ...
    return (
      ...
      >
        <div>
          ...
          >
            Asignar Artículos/Publicaciones
          </h1>
